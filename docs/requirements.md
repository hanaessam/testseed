...
| Preview and Editing | As a developer, I want to preview generated records before using them. | Generated records appear in per-collection tables. The user can inspect field values, ObjectId references, and validation status. | If a collection has no generated records, the system shows an empty state and allows the user to regenerate. |
| Preview and Editing | As a developer, I want to make direct edits to generated records. | The user can edit table cells before export or insertion. The system revalidates edited records. Invalid edits are highlighted. | If an edit violates a type, enum, required field, or uniqueness rule, the system prevents final export or insertion until the issue is fixed. |
...
6. **Feedback Regeneration Screen:** This screen places a feedback comment box beside or below the preview. Example comments may include "make users Canadian," "create more realistic product names," or "make order totals match item prices." The regenerate action should clearly indicate that the previous output and comments will both be considered.
