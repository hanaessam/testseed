...
**Generation UX (shipped):** New projects use a **setup wizard** (project context → optional GitHub → schema input → review). After the schema is saved, users open the **Generation Workbench** for counts, generation, table preview, **dataset versions**, and streamed AI refinement chat. Returning users with a saved schema can open the workbench directly (`?mode=generate`). Each generation, refinement, manual edit, or accepted candidate creates an **immutable dataset version** with optional `versionLabel` and `parentDatasetId` lineage. Users load any version into the workbench or **re-seed** it to MongoDB after confirmation. See `docs/dataset-version-history.md`, `docs/ui-design.md` § Generate Flow, and `docs/generation-ux-roadmap.md`.

**Dataset versions vs seed batches:** Immutable generated dataset versions are **shipped** (`docs/dataset-version-history.md`). Each generation, dataset-changing refinement, accepted candidate, or manual save creates or forks a persisted generated dataset record. **Seed batch rollback** is separate: it deletes MongoDB records by `seedBatchId`. See `docs/shipped-features.md` for the full inventory.
...
| Preview and Editing | As a developer, I want to preview generated records before using them. | **Shipped (006–007):** Generated records appear in per-collection tables in the workbench. The user can inspect field values, ObjectId references, and validation status. **Dataset versions** restore preview state, collection counts, and refinement chat history. | If a collection has no generated records, the system shows an empty state and allows the user to regenerate. |
| Preview and Editing | As a developer, I want to make direct edits to generated records. | **Shipped (007):** The user can edit table cells before export or insertion. Saves **fork** a new dataset version (they do not overwrite the prior version). The system revalidates edited records. Invalid edits are highlighted; export and direct seed stay blocked until valid. | If an edit violates a type, enum, required field, or uniqueness rule, the system prevents final export or insertion until the issue is fixed. |
| Dataset Version History | As a developer, I want every refinement and edit saved as a version so I can return to earlier data even within the same session. | **Shipped:** Refine/regenerate snapshots `Before refine: …` before applying changes. Successful refinements, manual saves, and accepted candidates create labeled versions with `parentDatasetId`. The workbench **Dataset versions** panel lists all versions chronologically. | If no `savedDatasetId` is active yet, the first save creates the initial version without a parent. |
| Dataset Version History | As a developer, I want to load any prior version and re-seed it to MongoDB after confirmation. | **Shipped:** Select a version to load in the workbench. **Re-seed** opens a confirmation dialog, then direct-seeds that version's data (requires a tested MongoDB connection from Export). | If no connection was tested, the UI prompts the user to open Export and test the connection first. |
...
| Rollback | As a developer, I want to rollback a seed batch. | The user can roll back by `seedBatchId` (Export drawer seed history or project history). The system deletes only records tagged with that batch and reports deleted counts by collection. **Note:** Restoring earlier *generated data* uses **dataset versions** and re-seed, not seed batch rollback. | If the batch ID is missing, invalid, or already rolled back, the system shows an error and does not delete unrelated data. |
...
flowchart LR
    ...
    API --> Versions[Dataset Version Service]
...
The backend sends the project description, schema structure, and record counts to the AI generation service. The AI returns generated records grouped by collection. The backend validates the records and returns them to the frontend for preview. If the user uses the AI refinement chat box, the backend snapshots the current dataset as a **Before refine** version (when an active saved version exists), sends the previous generated records, schema structure, validation context, and the user's message to the AI service, and persists the outcome as a new **dataset version** when the dataset changes. Every refined dataset is revalidated before it can replace the current valid workbench state. Once the user is satisfied, the final dataset can be exported as JSON, exported as a JavaScript seed script, or inserted directly into MongoDB after confirmation — either from the active workbench dataset or by **re-seeding** any saved version. Directly inserted records receive a `seedBatchId`, which allows the user to roll back that MongoDB batch later.
