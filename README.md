...
After manual parsing or MongoDB discovery, the Generate screen shows a schema review with collection tabs, field rules, confidence markers, enum-like values, reference evidence, nested fields, and warnings. Save the reviewed schema to attach it to the active project snapshot before continuing to generation.
